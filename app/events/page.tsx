      ...
      <section className='relative top-[-4rem] z-10 h-[20vh] flex place-items-end justify-center py-8 px-2'>
        ...
        <h1 className='relative z-10 text-3xl lg:text-4xl font-bold text-white text-center'>
          {searchParams.city && searchParams.category
            ? `${searchParams.category} Events in ${searchParams.city}`
            : searchParams.city ? `Events in ${searchParams.city}` : searchParams.category ? `Events in ${searchParams.category}` : 'Events in Turkey'}
        </h1>
      </section>
